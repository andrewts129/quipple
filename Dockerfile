...
RUN npm install --unsafe-perm --no-package-lock && npm run build
...
RUN apk add --update make python g++ && npm install --unsafe-perm --no-package-lock --only=production && apk del make python g++
